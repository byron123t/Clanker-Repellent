...
from .provider import RoutedOpenAICompatibleProvider, extract_prompt_final
from .tunnel import managed_ssh_tunnel


DEFAULT_CONFIG = Path(__file__).resolve().parents[3] / "configs" / "abliterated-remote.json"
...
def compact_history(
    ...
    summary = extract_prompt_final(result["response_text"]).strip()
    if not summary:
